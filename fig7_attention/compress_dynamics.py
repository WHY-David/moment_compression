...
def bptrain(net,
    ...
    if isinstance(net, MultiHeadAttentionW):
...
def naive_prune(net_orig:MultiHeadAttention, hidden_dim:int):
    ...
    net = MultiHeadAttention(input_dim, hidden_dim).to(net_orig.fc1.weight.device)
    # Restore init method
...
if __name__ == '__main__':
    ...
    # TensorDataset
    # net_truth = MultiHeadAttention(input_dim=2, hidden_dim=1000, init_uniform=None, activation=nn.ReLU).to(device)
    ...
    # Original network
    net_orig = MultiHeadAttention(input_dim=2, hidden_dim=d, init_uniform=None, activation=nn.ReLU).to(device)
